...
function setServerOnline() {
  ...
  document.getElementById('statusText').textContent = 'เชื่อมต่อ server แล้ว';
}
...
async function fetchTasks() {
  ...
  try {
    ...
    allTasks = data.data;
    setServerOnline(); // Set server status to online (🟢)
    renderTasks();
  } catch (error) {
    console.error("Error fetching tasks:", error);
    setServerOffline(); // Set server status to offline (🔴)
